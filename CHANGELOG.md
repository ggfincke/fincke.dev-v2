...
- Added 456code to the projects archive with public repository attribution and current technical details

### Changed

- Refreshed Westinghouse experience, About copy, SEO metadata, and current project information
- Updated the linked resume PDF to the active Spring 2027 version

## [2.10.1] - 2026-07-24

### Changed

- About copy now leads with the Westinghouse Software Developer Intern role and document-retrieval focus, after Scale AI
- Reworded Westinghouse and Scale AI experience blurbs; dropped `postgresql` from the Scale AI tech tags
...
- Stabilized the Lighthouse browser CI gate by auditing preview output with desktop/provided-throttling settings, avoiding shared-runner CPU simulation flakes while preserving the route score thresholds

## [2.9.0] - 2026-05-15

### Changed

- Migrated the project runtime and package manager to Bun 1.3 (`packageManager: bun@1.3.13`); dropped `tsx` in favor of Bun's native TypeScript execution and replaced `package-lock.json` with `bun.lock`
- CI, content-health, and release workflows now provision Bun via `oven-sh/setup-bun` and run `bun ci` / `bun run` scripts
- Release workflow now promotes stable releases by pushing the tagged commit directly to `prod` (latest-stable-tag and matching-tree guards) instead of opening a promotion pull request
